...
# >>>>>>>>>>>>>>>>> train_sp : spit4/4/feat.scp
if [ "$speed_perturb" == "true" ]; then
  if [ $stage -le 1 ]; then
    for lang_index in `seq 0 $[$num_langs-1]`; do
      for datadir in train; do
        if [ ! -d data/${lang_list[$lang_index]}/${datadir}_sp ]; then
          ./utils/data/perturb_data_dir_speed_3way.sh data/${lang_list[$lang_index]}/${datadir} data/${lang_list[$lang_index]}/${datadir}_sp
          
          # Extract Plp+pitch feature for perturbed data
          featdir=plp_perturbed/${lang_list[$lang_index]}

          if $use_pitch; then
            steps/make_plp_pitch.sh --cmd "$train_cmd" --nj $nj  data/${lang_list[$lang_index]}/${datadir}_sp exp/${lang_list[$lang_index]}/make_plp_pitch/${datadir}_sp $featdir
          else
            steps/make_plp.sh --cmd "$train_cmd" --nj $nj data/${lang_list[$lang_index]}/${datadir}_sp exp/${lang_list[$lang_index]}/make_plp/${datadir}_sp $featdir
          fi

          steps/compute_cmvn_stats.sh data/${lang_list[$lang_index]}/${datadir}_sp exp/${lang_list[$lang_index]}/make_plp/${datadir}_sp $featdir || exit 1;
          utils/fix_data_dir.sh data/${lang_list[$lang_index]}/${datadir}_sp
  ...
  if [ $stage -le 2 ] && [ "$generate_alignments" == "true" ]; then
    for lang_index in `seq 0 $[$num_langs-1]`; do
      ...
      steps/align_fmllr.sh \
        ...
        data/${lang_list[$lang_index]}/$train_set data/${lang_list[$lang_index]}/lang exp/${lang_list[$lang_index]}/tri5 exp/${lang_list[$lang_index]}/tri5_ali_sp || exit 1
    done
  fi
fi
...
if [ $stage -le 3 ]; then
  for lang_index in `seq 0 $[$num_langs-1]`; do
    ...
    mfccdir=mfcc_hires/${lang_list[$lang_index]}
    mfcc_affix=""
    if $use_pitch; then
      hires_config="$hires_config --online-pitch-config $pitch_conf"
      mfccdir=mfcc_hires_pitch/${lang_list[$lang_index]}
      mfcc_affix=_pitch_online
    fi
  

    for dataset in $train_set ; do
      data_dir=data/${lang_list[$lang_index]}/${dataset}${feat_suffix}
      log_dir=exp/${lang_list[$lang_index]}/make${feat_suffix}/$dataset

      utils/copy_data_dir.sh data/${lang_list[$lang_index]}/$dataset ${data_dir} || exit 1;
...
for lang_index in `seq 0 $[$num_langs-1]`; do
  ...
  multi_egs_dirs[$lang_index]=exp/${lang_list[$lang_index]}/nnet3${nnet3_affix}/egs${feat_suffix}
...
if [ $stage -le 9 ]; then
  ...
  ### Config Section for Prepare Multilingual Egs
  get_egs_stage=0
...
if [ $stage -le 13 ]; then
  ...
  dataset_dir=$data/$decode_data_dir
  dataset_id=$decode_data_dir
